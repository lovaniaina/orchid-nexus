...
function LoginPage({ onLoginSuccess }) {
  ...
  const handleAuth = async (event) => {
    ...
    if (isLoginView) {
      ...
      try { const response = await axios.post('http://localhost:8000/token', formData); onLoginSuccess(response.data.access_token); }
      catch (err) { setError('Login failed. Please check credentials.'); } finally { setIsLoading(false); }
    } else {
      try { await axios.post('http://localhost:8000/users/', { email, password, role }); setError('Account created! Please log in.'); setIsLoginView(true); }
...
function ProjectSelectionPage({ onSelectProject, onLogout, currentUser }) {
  ...
  const fetchProjects = useCallback(async () => { try { const res = await axios.get('http://localhost:8000/projects/'); setProjects(res.data); } catch (err) { console.error(err); } }, []);
  useEffect(() => { fetchProjects(); }, [fetchProjects]);
  const handleCreateProject = async (name) => { try { await axios.post('http://localhost:8000/projects/', { name }); fetchProjects(); } catch (err) { console.error('Failed to create project', err); } };
...
function KpiHistoryModal({ kpi, onClose }) {
    ...
    useEffect(() => { axios.get(`http://localhost:8000/kpis/${kpi.id}/history`).then(res => setHistory(res.data)); }, [kpi.id]);
...
function KpiIndicator({ kpi, currentUser }) {
    ...
    const handleDelete = async () => { if (window.confirm(`Delete KPI: "${kpi.name}"?`)) { try { await axios.delete(`http://localhost:8000/kpis/${kpi.id}`); } catch (err) { console.error(err); } } };
...
function KpiEntryForm({ kpi }) {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); try { await axios.post(`http://localhost:8000/kpis/${kpi.id}/entries`, { value: parseInt(value, 10) }); setValue(''); } catch (err) { console.error(err); } };
...
function AddKpiForm({ activityId }) {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); try { await axios.post('http://localhost:8000/kpis/', { name, unit, target_value: parseInt(target, 10) || 0, activity_id: activityId }); setName(''); setUnit(''); setTarget(100); } catch (err) { console.error(err); } };
...
function DeliverableForm({ task }) {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); const formData = new FormData(); formData.append('task_id', task.id); if (textContent) formData.append('text_content', textContent); if (proofFile) formData.append('proof_file', proofFile); try { await axios.post('http://localhost:8000/deliverables/', formData); setTextContent(''); setProofFile(null); e.target.reset(); } catch (err) { console.error(err); } };
...
function TaskItem({ task, currentUser }) {
    ...
    const handleToggleStatus = async () => { try { await axios.patch(`http://localhost:8000/tasks/${task.id}/status`); } catch (err) { console.error(err); } };
    const handleDelete = async () => { if (window.confirm(`Delete task: "${task.description}"?`)) { try { await axios.delete(`http://localhost:8000/tasks/${task.id}`); } catch (err) { console.error(err); } } };
...
function AddTaskForm({ activityId, users }) {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); const taskData = { description, activity_id: activityId, assignee_id: assigneeId ? parseInt(assigneeId, 10) : null, start_date: startDate || null, end_date: endDate || null }; try { await axios.post('http://localhost:8000/tasks/', taskData); setDescription(''); setAssigneeId(''); setStartDate(''); setEndDate(''); } catch (err) { console.error(err); } };
...
function ActivityPanel({ activity, users, currentUser, filter }) {
    ...
    const handleDelete = async (e) => { e.stopPropagation(); if (window.confirm(`Delete Activity: "${activity.name}"?`)) { try { await axios.delete(`http://localhost:8000/activities/${activity.id}`); } catch (err) { console.error(err); } } };
    return (
...
function ObjectivePanel({ objective, users, currentUser, filter }) {
    ...
    const isManager = currentUser.role === 'Project Manager';
    const handleActivitySubmit = async (name) => { try { await axios.post('http://localhost:8000/activities/', { name, objective_id: objective.id }); } catch(err) { console.error(err); } };
    const handleDelete = async (e) => { e.stopPropagation(); if(window.confirm(`Delete Objective: "${objective.name}"?`)) { try { await axios.delete(`http://localhost:8000/objectives/${objective.id}`); } catch (err) { console.error(err); } } };
    return (
...
function DashboardPage({ project, onDataChange, onBack, onLogout, users, currentUser }) {
    ...
    const fetchSummaryData = useCallback(async () => { try { const res = await axios.get(`http://localhost:8000/projects/${project.id}/summary`); setSummaryData(res.data); } catch (error) { console.error(error); } }, [project.id]);
    useEffect(() => { fetchSummaryData(); ws.current = new WebSocket(`ws://localhost:8000/ws/${project.id}`); ws.current.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === 'notification') { setNotification(data.message); onDataChange(); }}; return () => { ws.current.close(); }; }, [project.id, onDataChange, fetchSummaryData]);
    const handleObjectiveSubmit = async (name) => { try { await axios.post('http://localhost:8000/objectives/', { name, project_id: project.id }); } catch (err) { console.error(err); } };
...
function App() {
  ...
  useEffect(() => { if (token && !currentUser) { setIsLoading(true); Promise.all([axios.get('http://localhost:8000/users/me'), axios.get('http://localhost:8000/users/')]).then(([meRes, usersRes]) => { setCurrentUser(meRes.data); setAllUsers(usersRes.data); }).catch(() => handleLogout()).finally(() => setIsLoading(false)); } }, [token, currentUser, handleLogout]);
  const fetchProjectData = useCallback(async () => { if (!selectedProjectId || !token) return; setIsLoading(true); try { const response = await axios.get(`http://localhost:8000/projects/${selectedProjectId}`); setProjectData(response.data); setError(''); } catch (err) { setError('Failed to load project data.'); if (err.response?.status === 401) handleLogout(); } finally { setIsLoading(false); } }, [selectedProjectId, token, handleLogout]);
