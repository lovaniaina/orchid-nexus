...
function DashboardPage({ project, onDataChange, onBack, onLogout, users, currentUser }) {
    ...
    useEffect(() => { fetchSummaryData(); ws.current = new WebSocket(`ws://orchid-nexus-backend.onrender.com/ws/${project.id}`); ws.current.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === 'notification') { setNotification(data.message); onDataChange(); }}; return () => { ws.current.close(); }; }, [project.id, onDataChange, fetchSummaryData]);
